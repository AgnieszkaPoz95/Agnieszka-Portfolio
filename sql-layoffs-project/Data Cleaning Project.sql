...
-- Approach 1: CTE with ROW_NUMBER() — used for duplicate identification.
-- This approach is clean and readable, but MySQL does not allow
-- DELETE directly from a CTE, so it can only be used to preview duplicates.
...
-- Approach 2: Staging table with row_num as a physical column.
-- This approach allows both identification AND deletion of duplicates,
-- which is why it was chosen as the final solution.
